...
| Total Deaths | `deaths` | INTEGER | counter | Any survivor death: `PlayerHealth.Die` (PULSE to 0) or zombie death via damage/starvation short of redemption (`PlayerTransformation.ResolveRageZero` death branches). Includes true deaths. → `+1` |
| Total True Deaths | `true_deaths` | INTEGER | counter | Permadeath only — the two documented True Death paths: human eaten (brain/devoured) or the player-zombie killed by anyone. Survivor deleted. → `+1` (and `deaths +1`) |
...
| Longest Survival Time | `longest_life_seconds` | INTEGER | max (seconds) | On any survivor end (death, true death, or turn closes the *human* stint — measured spawn → end of that survivor's life). → `max(stored, reported)` |
| Total Playtime | `playtime_seconds` | INTEGER | counter (seconds) | Session end / periodic flush: seconds played this session across all survivors. → `+= reported` |
...
| Lives | `lives` | INTEGER | counter | A survivor is started — the first spawn and every restart after a True Death (the "new survivor" flow). Incremented automatically when the game creates a survivor through the ingest API. → `+1` |
...
- **Counters** (`humans_killed`, `zombies_killed`, `times_turned`, `deaths`,
  ...
  survivor, not the account's lifetime tally.
...
Each account plays many survivors over time — permadeath means every True
Death ends one and the player rolls a new survivor. Alongside the aggregate
`player_stats` row, the site keeps one `survivors` row per survivor
...
- `name` (optional), `skin` (required) — which survivor/skin was played.
- `started_at`, `ended_at`, `outcome` — lifecycle; `outcome` is free text
  for now (e.g. `died` / `turned` / `true_death`), deliberately not
  over-modeled.

Creating a survivor through the API **automatically increments `lives`** —
a new survivor IS a new life, so the game never reports `lives` directly.
Per-survivor stat columns can come later; user-level aggregates stay in
`player_stats`. Stat reports may carry a `survivor_id`/`survivor_ref`
context, which is validated (must belong to the user) but not yet
aggregated per survivor.
...
Built: `POST /api/stats` (stat reports + survivor create/end) and a `GET`
